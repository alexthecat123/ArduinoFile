...
//10M Spare Table
//byte spareTable[26]   = {0x50, 0x52, 0x4F, 0x46, 0x49, 0x4C, 0x45, 0x20, 0x31, 0x30, 0x4D, 0x20, 0x20, 0x00, 0x00, 0x10, 0x04, 0x04, 0x00, 0x4C, 0x00, 0x02, 0x14, 0x20, 0x00, 0x00};

char fileName[256];

// lookup table to store the parity of each index of the table.
// The macro `FIND_PARITY` generates the table
/*byte parityLookup[256] = {0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0,
0x1, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1,
0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1, 0x0,
0x1, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0,
0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0,
0x1, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1,
0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x0,
0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1, 0x0, 0x1, 0x0, 0x0, 0x1,
0x1, 0x0, 0x0, 0x1, 0x0, 0x1, 0x1};*/

byte parityLookup[256] = { FIND_PARITY };
...
//volatile byte *pointer; //a pointer that we use for the above arrays (it makes code execution faster than doing array[index])
...
void setup() {
  ...
  //Serial.println(pointerTest);
  /*for(int i = 0; i < 255; i++){
    Serial.print("0x");
    Serial.print(parityLookup[i]);
    Serial.print(", ");
  }*/
...
void loop() {
  ...
  byte *parityPointer = parityLookup;
  unsigned int value;
  ogPointer = pointer;
  //EIMSK |= (1 << INT4);
  //EIFR |= (1 << INT4); //enable INT4 and clear the interrupt flag in case it's already been triggered (this happens sometimes for some reason and screws things up if you don't clear the flag)
  PORTC = PORTC | B00000010; //if everything checks out, raise BSY
  //sei(); //enable interrupts
  //while((PINC & B00000001) == B00000001); //and service interrupts triggered by the strobe while we wait for CMD to go low (see the ISR(INT4_vect) function below)
  //the interrupts will store the received data in the commandBuffer array
  //the strobe pulses produced by NeoWidEx during the command phase are much smaller than the pulses produced in the other phases, so we have to use interrputs to catch them here, but not in the other phases
  //000001C56414
  //cli(); //after CMD goes low, meaning that we have all the command bytes, disable all interrupts again
  //EIMSK &= (0 << INT4); //disable INT4
  //startTime = millis();
  currentTime = 0;
  byte oldPIN = 0;
  while((PINC & B00000001) == B00000001){ //do this for each of the remaining data bytes and ((PINC | B11111011)) == B11111011
    ...
    /*if(PINL != oldPIN){
      oldPIN = PINL;
      if((*(parityPointer + oldPIN) == 0)){ //does this work???
        //PORTC = PORTC | B00100000;
      }
      else{
        //PORTC = PORTC & B11011111;
...
void readDrive(){
  ...
  if(commandBuffer[1] == 0xFF and commandBuffer[2] == 0xFF and commandBuffer[3] == 0xFF){
    ...
    //Emu: 1 min 8 sec; Real: 1 min 0 sec
    disk.seekSet(byteNum);
    disk.read(data, 532);
    /*for(int i = 0; i < 532; i++){
      printDataSpace(data[i]);
    }*/
    //delay(10);
    delay(1);
    cli();
  ...
  /*while(index < 3){ //do this for each of the remaining 3 status bytes
    currentState = PINC & B00001000;
    if(currentState == B00000000 and prevState == B00001000){ //if we're on the falling edge of the strobe, put the next status byte on the bus and increment index
      PORTL = 0x00;
      index += 1;
    }
    prevState = currentState;
...
/*ISR(INT4_vect){ //the ISR for reading the command bytes
  *pointer = PINL; //save the byte on the bus to the address currently pointed at by the pointer
  pointer++; //and increment the pointer
}

ISR(INT5_vect){ //currently not used (I couldn't get it to work properly for some reason and the pointer kept randomly changing in value for no reason)
  PORTL = *pointer;
  pointer++;
}*/
